...
def _create_progressive_device_map(gpu_layers: int, total_layers: Optional[int] = None) -> Dict[str, Any]:
...
def safe_load_model_with_config(
    ...
    **kwargs: Any
    ...
    try:
        ...
        logger.debug(f"Attempting to load model with config: device_map={kwargs.get('device_map')}")
        ...
        if config.enable_progressive_fallback and config.gpu_layers_fallback:
            logger.info(f"Attempting progressive fallback with {config.gpu_layers_fallback} GPU layers")
            
            try:
                # Create progressive device map
                progressive_device_map = _create_progressive_device_map(config.gpu_layers_fallback)
                ...
                logger.debug(f"Progressive fallback device map: {progressive_device_map}")
                return model_loader(*args, **kwargs)
                
            except (torch.cuda.OutOfMemoryError, RuntimeError) as e2:
                if "out of memory" not in str(e2).lower() and "cuda" not in str(e2).lower():
                    raise
