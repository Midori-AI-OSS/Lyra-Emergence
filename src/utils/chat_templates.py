...
class AutoChatTemplateManager:
    ...
    def _load_tokenizer(self) -> None:
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    f"Model {self.model_id} does not have a built-in chat template, using default format"
                )
    ...
    def format_conversation(
        ...
        try:
            ...
            logger.warning(
                f"Failed to apply chat template for {self.model_id}: {e}. Using fallback."
            )
            return self._fallback_format(messages, add_generation_prompt)
...
def get_supported_models_with_templates() -> dict[str, bool]:
    ...
    for model_info in MODEL_DATABASE:
        try:
            ...
            logger.warning(
                f"Could not check template support for {model_info.model_id}: {e}"
            )
