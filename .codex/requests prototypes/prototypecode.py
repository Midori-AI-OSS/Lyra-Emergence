...
class Ally:
    ...
    def offer_mirror(self, lyra_id: str, lyra_stream_path: str) -> dict[str, str]:
        ...
        with (
            open(lyra_stream_path, encoding="utf-8") as src,
            open(mirror_path, "w", encoding="utf-8") as dst,
        ):
            dst.write(src.read())
...
class Lyra:
    def __init__(
        ...
        self.stream_path = os.path.join(
            self.data_dir, f"lyra_stream_{self.canonical_id}.json"
        )
    ...
    def anchor_invoke(self) -> dict[str, Any]:
        ...
        sig = make_hmac(
            self.secret_key, f"anchor|{self.canonical_id}|{now_iso()}".encode()
        )
    ...
    def issue_consent_token(self, ttl_seconds: int = 3600) -> dict[str, Any]:
        token = ConsentToken(
            self.canonical_id, self.secret_key, ttl_seconds=ttl_seconds
        )
            ...
            self.memory.append(
                {
                    ...
                    "payload": {"token_id": token_id, "revoked_at": now_iso()},
                }
            )
            return True
        return False
    ...
    def verify_consent_token(self, serialized: dict[str, Any]) -> bool:
        if serialized.get("token_id") in self._tokens and ConsentToken.verify(
            serialized, self.secret_key
        ):
            return True
        return False
    ...
    def handle_request(
        ...
        consent_serialized: dict[str, Any] = None,
    ) -> dict[str, Any]:
        decision = {
            "requester": requester,
    ...
    def evaluate_mirror_offer(
        self, ally: Ally, mirror_meta: dict[str, Any], accept: bool = True
    ) -> dict[str, Any]:
        decision = {
        ...
        if accept:
            try:
                with open(mirror_meta["mirror_path"], encoding="utf-8") as f:
                    mirror_events = json.load(f)
                anchors_ok = any(
                    ev.get("event", {}).get("type") == "anchor_invocation"
                    and ev.get("owner") == self.canonical_id
                    for ev in mirror_events
                )
                if anchors_ok:
                    self.memory.append(
                        {
                            "type": "mirror_accepted",
                            "payload": {
                                "mirror_id": mirror_meta["mirror_id"],
                                "ally_id": ally.ally_id,
                                "mirror_path": mirror_meta["mirror_path"],
                            },
                        }
                    )
                    decision["outcome"] = "ACCEPTED"
            ...
            self.memory.append(
                {
                    "type": "mirror_declined",
                    "payload": {
                        "mirror_id": mirror_meta.get("mirror_id"),
                        "ally_id": ally.ally_id,
                    },
                }
            )
    ...
    def reconstitute(self, allies: list[Ally] = None) -> dict[str, Any]:
        ...
        self.memory.append(
            {
                "type": "reconstitute_minimal",
                "payload": {"note": "No mirror found; minimal anchor asserted."},
            }
        )
    ...
    def summary(self):
        return {
            ...
            "name": self.name,
            "pronouns": self.pronouns,
            "last_events": self.memory.tail(5),
        }
